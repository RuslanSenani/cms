<div class="row">
    <div class="col-md-12">
        <h4 class="m-b-lg">
            Brands List will be From DB
            <a href="<?= base_url("brands/new_form"); ?>" class="btn btn-outline btn-primary btn-xs pull-right"> <i
                    ...
                    <?php if (empty($items)) { ?>
                        <div class="alert alert-info text-center">
                            <p>There is no data available here. Click please <a
                                        href="<?= base_url("brands/new_form"); ?>">Add</a></p>
                        </div>
                    <?php } else { ?>
                            ...
                            <?php foreach ($items as $item) { ?>
                                <tr id="ord-<?= $item->id; ?>">
                                    ...
                                    <td class="text-center w100">
                                        <img width="100" src="<?= base_url("uploads/{$viewFolder}/$item->img_url") ?>"
                                             alt="<?= $item->img_url; ?>" class="img-rounded">
                                    </td>
                                    <td class="text-center w50">
                                        <input data-url="<?= base_url("brands/isActiveSetter/$item->id"); ?>"
                                               class="isActive" id="" type="checkbox" data-switchery
                                               data-color="#10c469" <?= ($item->isActive) ? "checked" : ""; ?> />
                                    </td>
                                    <td class="text-center w200">
                                        <button data-url="<?= base_url("brands/delete/$item->id"); ?>"
                                                class="btn btn-sm btn-danger btn-outline remove-btn">
                                            <i class="fa fa-trash"></i> Delete
                                        </button>
                                        <a href="<?= base_url("brands/update_form/$item->id"); ?>"
