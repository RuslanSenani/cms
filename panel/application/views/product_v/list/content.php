<div class="row">
    <div class="col-md-12">
        <h4 class="m-b-lg">
            Product List will be From DB
            <a href="<?= base_url("product/new_form"); ?>" class="btn btn-outline btn-primary btn-xs pull-right"> <i
            ...
            <div class="widget-body">
                ...
                <?php if (empty($items)) { ?>
                    <div class="alert alert-info text-center">
                        <p>There is no data available here. Click please <a href="<?= base_url("product/new_form"); ?>">Add</a>
                        </p>
                    </div>
                <?php } else { ?>
                        ...
                        <?php foreach ($items as $item) { ?>
                            <tr id="ord-<?= $item->id; ?>">
                                ...
                                <td class="text-center">
                                    <input data-url="<?= base_url("product/isActiveSetter/$item->id"); ?>"
                                           class="isActive" id="" type="checkbox" data-switchery
                                           data-color="#10c469" <?= ($item->isActive) ? "checked" : ""; ?> />
                                </td>
                                <td class="text-center">
                                    <button data-url="<?= base_url("product/delete/$item->id"); ?>"
                                            class="btn btn-sm btn-danger btn-outline remove-btn">
                                        <i class="fa fa-trash"></i> Delete
                                    </button>
                                    <a href="<?= base_url("product/update_form/$item->id"); ?>"
                                       class="btn btn-sm btn-success btn-outline"> <i class="fa fa-pencil-square-o"></i>
                                        Update</a>
                                    <a href="<?= base_url("product/image_form/$item->id"); ?>"
